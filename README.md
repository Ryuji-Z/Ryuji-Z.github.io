...
**概要：**  
　測位航法学会が主催する、GNSS のみでロボットを自律移動させるコンペティションに2023-2025年の3年間参加しました。  
　約 10 校の大学・高専が参加する中、2023年と2024年に**大会 2 連覇**を達成しました。
...
**概要：**  
　共同研究を兼ねて、つくば市内の街中を約 2 km 自律移動する実証実験・技術交流の場に3年間参加しました。<br>
　仲間と協力し３年目にはコースの半分である約 1 km の自律移動を達成しました。大学院においても周囲を巻き込み、<br>
　参加に向けて活動しています。
...
- GNSS、IMU、エンコーダおよびLiDAR のセンサフュージョン
- 画像認識モデルを用いた信号機・文字認識
...
<details>
<summary><strong>個人制作：ゲーム制作</strong></summary>

動物タワーバトルを模倣した、人間タワーバトルを制作しました。  
PCのカメラでリアルタイムにキャプチャした画像をブロックとして扱い、積み上げていくゲームです。  
AIを活用し、3日ほどで完成させました。

<p align="center">
  <img src="https://img.youtube.com/vi/A-nT3PhVn_c/maxresdefault.jpg" alt="人間タワーバトルのプレイ画面" width="420"><br>
  <sub>人間タワーバトルのプレイ画面</sub>
</p>

`Python` `OpenCV` `MediaPipe` `Pygame` `Pymunk` `NumPy` `Webカメラ処理` `リアルタイム画像処理` `物理演算`

▶ [プレイ動画](https://youtu.be/A-nT3PhVn_c)

</details>

<details>
<summary><strong>個人制作：GUIグラフ作成ツール</strong></summary>

CSV / XLSX データからグラフを作成し、軸・凡例・目盛・配色などをGUI上で調整できるツールを作製しました。  
開発期間は2週間程度です。Pythonで実装し、PyInstallerを用いてexe化しました。  
Tkinterの標準UIは見た目が古く感じたため、CustomTkinterを使い、モダンなUIになるようにこだわりました。

<p align="center">
  <img src="assets/gui-graph-tool.png" alt="GUIグラフ作成ツールの画面" width="280"><br>
  <sub>GUIグラフ作成ツールの画面</sub>
</p>

`Python` `CustomTkinter` `PyInstaller` `CSV/XLSX` `グラフ描画` `データ可視化`
...
[YouTube Channel](https://www.youtube.com/channel/UCtUk-U6MxWCbHHhmOjC0xRw)
